...
import {FlexBox, onlyMobile, Section} from "../../../assets/style/Layout.style";
...
const NotFound = ({ history }) => {
    ...
    return (
        <section css={css` ${Section()} ${FlexBox('column','center','center')} `}>
            <div css={css`${Display2('',500)}; margin-bottom:20px; text-align: center; word-break: keep-all;`}>죄송합니다. 현재 찾을 수 없는 페이지를 요청하셨습니다.</div>
            <div css={css`${MarkdownBase('#666')}; text-align:center; margin-bottom:50px;`}>
                존재하지 않는 주소를 입력하셨거나,<br/>
                요청하신 페이지의 주소가 변경, 삭제되어 찾을 수 없습니다.
            </div>
            <div css={css`text-align: center;`}>
                <button css={css`margin-right:10px; margin-bottom:10px; ${MarkdownMdx(Color.white)}; width: 215px; padding:10px 0; background: #951DE4; border-radius: 5px;`} onClick={goMain}>메인으로</button>
